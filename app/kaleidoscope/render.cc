...
auto static viewport_and_scissor_rect(
    ComPtr<IDXGISwapChain3> const & swap_chain, D3D12_VIEWPORT & viewport, D3D12_RECT & scissor_rect
) -> HRESULT
{
...
auto static render_target_views(
    ComPtr<ID3D12Device> const & device, ComPtr<IDXGISwapChain3> const & swap_chain,
    ComPtr<ID3D12DescriptorHeap> &          render_target_view_descriptor_heap, /* out */
    std::array<ComPtr<ID3D12Resource>, 2> & render_targets                      /* out */
) -> HRESULT
{
    // Refer to:
...
public:
        ...
        , window_height(height)
    {
        ...
        auto constexpr feature_levels = std::array<D3D_FEATURE_LEVEL, 1>{D3D_FEATURE_LEVEL_11_0};
        ...
        // - Update descriptor size and frame index
        {
            ...
            make::render_target_views(device, swap_chain, render_target_view_descriptor_heap, render_targets) >>
                must::succeed;
        ...
        /// Create an IDXGIOutput
        {
            // Device and context
            D3D11CreateDevice(
                adapter.Get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, 0, feature_levels.data(),
                static_cast<UINT>(feature_levels.size()), D3D11_SDK_VERSION, &device11, nullptr, &context11
            ) >> must::succeed;
        ...
        // Refer to:
        // https://learn.microsoft.com/en-us/windows/win32/direct3d12/resource-binding-flow-of-control#resource-binding-flow-of-control
        {
            ...
            // We could use "device->CheckFeatureSupport(D3D12_FEATURE_ROOT_SIGNATURE, ...)" to check if 1_1 is
            // supported
        ...
        // https://github.com/microsoft/DirectX-Graphics-Samples/blob/a79e01c4c39e6d40f4b078688ff95814d166d34f/Samples/Desktop/D3D12HelloWorld/src/HelloConstBuffers/D3D12HelloConstBuffers.cpp
        {
            ...
            auto constexpr aligned_size =
                (size + 0xff) &
                ~0xff; // Required to be 256-byte (D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT) aligned
            ...
            device->CreateCommittedResource(
                &heap_properties, D3D12_HEAP_FLAG_NONE, &resource_desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
